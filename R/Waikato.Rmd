...
# This is results loading once the data has been generated. Not available to Luke right now
...
# Luke has set his in the R profile to 24gb
...
ttm_sparse_list <- ttm_car_results_list %>%
  map(~ .x %>%
         tidyr::pivot_wider(
           ...
           values_fill = 0  # or use Inf if that’s your default non-connection
         ) %>%
         {
           df <- .
...
process_area <- function(
  ...
  cat("  • Origins:", nrow(origins), "cells\n")
  cat("  • Destinations:", nrow(dests), "cells\n")
  ...
  cat("  • Running travel_time_matrix(", mode, ") …\n")
  ttm <- tryCatch(
    {
      travel_time_matrix(
        r5r_core_waikato,
    ...
    error = function(e) {
      message("    ✗ Error in travel_time_matrix for ", area_id, ": ", e$message, "\n")
      return(NULL)
    }
  )
  if(is.null(ttm)){
    cat("    → ttm FAILED for", area_id, "\n")
    return(NULL)
  } else {
    cat("    ✓ ttm complete for", area_id, "\n")
  }
  
  # save raw TTM
  ttm_file <- file.path(ttm_dir, paste0("ttm_", tolower(mode), "_", area_id, ".rds"))
  saveRDS(ttm, ttm_file)
  cat("  • Saved raw TTM to", ttm_file, "\n")
  
  # 3. build multipolygon map
  cat("  • Building multipolygon bins …\n")
  ...
  cat("  • Saved interactive map to", mapfile, "\n")
  
  # 4. compute 1k-jobs access
  cat("  • Calculating 1k-jobs access …\n")
  ...
  cat("  • Saved jobs access to", jobs_file, "\n")
  
  # 5. compute nearest-school access
  cat("  • Calculating school access …\n")
  ...
  cat("  • Saved school access to", school_file, "\n")
...
#download the Natural Earth NZ polygon (everything that’s land)
